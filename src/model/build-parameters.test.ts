...
describe('BuildParameters', () => {
  describe('create', () => {
    ...
    it('returns orchestrator soft timeout minutes from input', async () => {
      const mockValue = '295';
      jest.spyOn(OrchestratorOptions, 'orchestratorSoftTimeoutMinutes', 'get').mockReturnValue(mockValue);
      await expect(BuildParameters.create()).resolves.toEqual(
        expect.objectContaining({ orchestratorSoftTimeoutMinutes: 295 }),
      );
    });
  });
});
