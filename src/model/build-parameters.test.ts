...
describe('BuildParameters', () => {
  describe('create', () => {
    ...
    it('preserves orchestrator branch names with slashes', async () => {
      jest.spyOn(OrchestratorOptions, 'orchestratorBranch', 'get').mockReturnValue('feat/timeout');
      await expect(BuildParameters.create()).resolves.toEqual(
        expect.objectContaining({ orchestratorBranch: 'feat/timeout' }),
      );
    });
  });
});
